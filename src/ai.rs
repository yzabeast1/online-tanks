...
use crate::{
    blocking_calculated_activation_filter, card_stays_active, log_play_event,
    remove_matching_cards, take_post_play_messages_since,
};
        ...
        if let Some(mut pending) = game.pending_recycle.take() {
            ...
            if pending.awaiting_discards.is_empty() && is_ai_player(&pending.player) {
                ...
                if let Some(card) = recycled_card {
                    ...
                    game.push_server_chat_message(format!("{} recycled {}", recycler, card_name));
                }
...
pub fn run_ai_turns(game: &mut GameState) {
    while game.alive_player_count() > 1 {
        ...
        if is_ai_player(&current_player.name) {
            ...
            game.push_server_chat_message(format!("{} ended their turn", ended_turn_player_name));
            game.next_turn();
        } else {
            break;
        }
...
fn run_single_ai_turn(game: &mut GameState) {
    ...
    loop {
        let ready_index = game
            .active_cards
            .active_calculated_shootings
            .iter()
            .position(|active| active.owner == current_player_name && active.turns_remaining == 0);
        ...
        if card_id == "decision-missile" {
            req.headers_mut().insert(
                "decision_action",
                hyper::header::HeaderValue::from_static("draw"),
            );
            ...
            req.headers_mut().insert(
                "multistrike_allocations",
                hyper::header::HeaderValue::from_str(&allocations_str).unwrap(),
            );
        } else {
            if !targets.is_empty() {
                req.headers_mut().insert(
                    "target",
                    hyper::header::HeaderValue::from_str(&targets[0]).unwrap(),
                );
            }
        }
    ...
    while played_any && game.alive_player_count() > 1 {
        ...
        for i in 0..hand_len {
            ...
            if is_healing_card(&card) {
                ...
                let targets =
                    get_ai_attack_targets(game, &current_player_name, ShootingType::Quick);
                ...
                if let Some((c1, c2)) =
                    find_two_shooting_cards_to_discard(&game.players[player_index].hand, i)
                {
                    req.headers_mut().insert(
                        "discardcard",
                        hyper::header::HeaderValue::from_str(&c1).unwrap(),
                    );
                    req.headers_mut().insert(
                        "discardcardtwo",
                        hyper::header::HeaderValue::from_str(&c2).unwrap(),
                    );
                }
                ...
                if let Some(qc) =
                    find_opponent_active_calculated_shooting(game, &current_player_name)
                {
                    req.headers_mut().insert(
                        "queuedcard",
                        hyper::header::HeaderValue::from_str(&qc).unwrap(),
                    );
                }
                ...
                for t in &["Quick", "Calculated", "Boom"] {
                    ...
                    if !game
                        ...
                        .iter()
                        .any(|f| f.owner == current_player_name && f.filter_type == st)
                    {
                        choose_type = t;
                        break;
                    }
                ...
                req.headers_mut().insert(
                    "firing_filter_type",
                    hyper::header::HeaderValue::from_static(choose_type),
                );
                ...
                req.headers_mut().insert(
                    "radar_order",
                    hyper::header::HeaderValue::from_str(&order_str).unwrap(),
                );
            }
            ...
            if card.id == "more-ammo" {
                ...
                for j in 0..hand_len {
                    ...
                    if other_card.card_type.is_shooting() {
                        ...
                        if other_card.id == "crack"
                            || other_card.id == "dent"
                            || other_card.id == "stolen-parts"
                        {
                            let targets = get_ai_attack_targets(
                                game,
                                &current_player_name,
                                ShootingType::Quick,
                            );
                        ...
                        } else if other_card.id == "big-bomb"
                            || other_card.id == "small-bomb"
                            || other_card.id == "nuke"
                        {
                            let targets = get_ai_attack_targets(
                                game,
            ...
            if (card.can_be_played)(&card, game, &req) {
                ...
                let post_play_messages =
                    take_post_play_messages_since(game, chat_messages_len_before_play);
    ...
    for p in &game.players {
        if p.health > 0 && p.name != current_player_name {
            let blocked = game
                ...
                .active_firing_filters
                .iter()
                .any(|filter| filter.owner == p.name && filter.filter_type == shooting_type);
            if !blocked {
...
fn get_ai_steal_targets(game: &GameState, current_player_name: &str) -> Vec<String> {
    let mut opponents: Vec<&crate::structs::Player> = game
        .players
        .iter()
...
fn find_two_shooting_cards_to_discard(
    hand: &[Card],
    exclude_idx: usize,
) -> Option<(String, String)> {
    let shooting_indices: Vec<usize> = hand
        .iter()
        .enumerate()
...
fn find_opponent_active_calculated_shooting(
    game: &GameState,
    current_player_name: &str,
) -> Option<String> {
    game.active_cards
